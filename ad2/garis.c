...
{
	float dx = Absis(PAwal(L))-Absis(PAkhir(L));
	float dy = Ordinat(PAwal(L))-Ordinat(PAkhir(L));
...
/* Prekondisi : x1 != x2 */
{
	float dy = Ordinat(PAkhir(L))-Ordinat(PAwal(L));
	float dx = Absis(PAkhir(L))-Absis(PAwal(L));
